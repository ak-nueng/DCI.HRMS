...
{
    public class SubContractTimeCardService
    {
        private static readonly SubContractTimeCardService instance = new SubContractTimeCardService();
        ...
        private SubContractLeaveService lvrqSvr = SubContractLeaveService.Instance();
        private SubContractShiftService shSvr = SubContractShiftService.Instance();
        internal SubContractTimeCardService()
        {
            tnTimeCardDao = factory.CreateTimeCardDAO();
            emTimeCardDao = emFactory.CreateTimeCardDAO();
            tmrqType = factory.CreateDictionaryDao();
        }
        public static SubContractTimeCardService Instance()
        {
            return instance;
        }
        ...
        public EmployeeWorkTimeInfo GetEmployeeWorkingHour(string empCode, DateTime workDate)
        {
            ...
            DateTime ntSecondEndTd = DateTime.Parse(workDate.ToString("dd/MM/yyyy ") + ntWork.SecondEnd.Hour.ToString("00") + ":" + ntWork.SecondEnd.Minute.ToString("00"));
            DateTime ntSeconStartNd = DateTime.Parse(workDate.AddDays(1).ToString("dd/MM/yyyy ") + ntWorkNt.SecondStart.Hour.ToString("00") + ":" + ntWorkNt.SecondStart.Minute.ToString("00"));


            DateTime dtFirstEndTd = DateTime.Parse(workDate.ToString("dd/MM/yyyy ") + dtWork.FirstEnd.Hour.ToString("00") + ":" + dtWork.FirstEnd.Minute.ToString("00"));

            DateTime dtFirstEndNd = DateTime.Parse(workDate.AddDays(1).ToString("dd/MM/yyyy ") + dtWorkNt.FirstEnd.Hour.ToString("00") + ":" + dtWorkNt.FirstEnd.Minute.ToString("00"));
            DateTime dtSecondStartTd = DateTime.Parse(workDate.ToString("dd/MM/yyyy ") + dtWork.SecondStart.Hour.ToString("00") + ":" + dtWork.SecondStart.Minute.ToString("00"));
            DateTime dtSecondEndTd = DateTime.Parse(workDate.ToString("dd/MM/yyyy ") + dtWork.SecondEnd.Hour.ToString("00") + ":" + dtWork.SecondEnd.Minute.ToString("00"));
            ...
            if (timeCard != null)
            {
                ...
                {
                    DateTime cd = DateTime.Parse(item.CardDate.ToString("dd/MM/yyyy ") + item.CardTime);
                ...
                if (timeCardIn.Count != 0)
                {
                    ...
                    for (int i = 0; i < timeCardIn.Count; i++)
                    {
                        ...
                        for (int j = 0; j < timeCardOut.Count; j++)
                        {
                            ...
                            DateTime cdOut = DateTime.Parse(itemOut.CardDate.ToString("dd/MM/yyyy ") + itemOut.CardTime);
                            TimeSpan tm = cdOut - empwk.WorkFrom;

                            if (timeCardIn.Count > i + 1)
                            {
                                TimeCardInfo inNext = (TimeCardInfo)timeCardIn[i + 1];
                                DateTime cdNext = DateTime.Parse(inNext.CardDate.ToString("dd/MM/yyyy ") + inNext.CardTime);
                ...
                else
                {
                    ...
                    foreach (TimeCardInfo item in timeCardOut)
                    {
                        ...
                        DateTime cdOut = DateTime.Parse(item.CardDate.ToString("dd/MM/yyyy ") + item.CardTime);
            ...
            if (!empwk.TimeOk && empwk.Remark != "(ABSE)")
            {
                if (timeCardIn.Count == 2 && timeCardOut.Count == 0 || timeCardIn.Count == 0 && timeCardOut.Count == 2)
                {
                    ...
                    DateTime cd1 = DateTime.Parse(tm1.CardDate.ToString("dd/MM/yyyy ") + tm1.CardTime);
                    DateTime cd2 = DateTime.Parse(tm2.CardDate.ToString("dd/MM/yyyy ") + tm2.CardTime);
            ...
            if (empwk.Remark == "(ABSE)")
            {
                ...
                if (tmrqLs != null)
                {
                    foreach (TimeCardManualInfo tmItem in tmrqLs)
                    {
                        ...
                        DateTime tmFm = DateTime.Parse(tmItem.RqDate.ToString("dd/MM/yyyy") + " " + tmItem.TimeFrom);
                        empwk.WorkFrom = tmFm;
        ...
        /// <param name="ntWork"></param>
        /// <param name="dtWorkNt"></param>
        /// <param name="ntWorkNt"></param>
        /// <returns></returns>
        public EmployeeWorkTimeInfo GetEmployeeWorkingHour(string empCode, DateTime workDate , WorkingHourInfo dtWork, WorkingHourInfo ntWork,WorkingHourInfo dtWorkNt, WorkingHourInfo ntWorkNt )
        {
            EmployeeWorkTimeInfo empwk = new EmployeeWorkTimeInfo();
            ...
            if (timeCard != null)
            {
                foreach (TimeCardInfo item in timeCard)
                {
                    ...
                    //Console.WriteLine(cd.ToString("yyyy-MM-dd HH:mm:ss") + " > " + ntSecondEndTd.ToString("yyyy-MM-dd HH:mm:ss") + " < " + ntSeconStartNd.ToString("yyyy-MM-dd HH:mm:ss"));
                    //Console.WriteLine(cd.ToString("yyyy-MM-dd HH:mm:ss") + " > " + dtFirstEndTd.ToString("yyyy-MM-dd HH:mm:ss") + " < " + dtFirstEndNd.ToString("yyyy-MM-dd HH:mm:ss"));
            ...
            // if (empwk.TimeOk)
            {
                ...
                if (empwk.TimeOk)
                {
                    ...
                    if (empwk.Shift == "D")
                    {
                        ...
                        {
                            empwk.TimeOk = false;
                            //Console.WriteLine(empwk.WorkFrom.ToString("yyyy-MM-dd HH:mm:ss") + " < " + dtWork.FirstEnd.ToString("yyyy-MM-dd HH:mm:ss") );
