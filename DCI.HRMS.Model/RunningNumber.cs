...
{
	/// <summary>
	/// Summary description for RunningNumber.
	/// </summary>
	public class RunningNumber
	{
		...
		{
			get { 
                if (this.m_ActiveDate <= DateTime.Parse("01/01/1900", new CultureInfo("en-US")) || this.m_ActiveDate <= DateTime.MinValue)
                {
		...
		{
			get { 
                if (this.m_Date <= DateTime.Parse("01/01/1900", new CultureInfo("en-US")) || this.m_Date <= DateTime.MinValue)
                {
					return DateTime.Now;
                }
                else
        ...
        public bool IsMustReset(DateTime curDate)
        {
            bool isReset = false;

            int actY = Convert.ToInt32(this.ActiveDate.ToString("yy"));
            int curY = Convert.ToInt32(curDate.ToString("yy"));

            int actM = Convert.ToInt32(this.ActiveDate.ToString("MM"));
            int curM = Convert.ToInt32(curDate.ToString("MM"));
